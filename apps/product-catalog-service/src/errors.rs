...
impl From<rust_database_clients::ConfigError> for ServiceError {
    fn from(err: rust_database_clients::ConfigError) -> Self {
        match err {
            rust_database_clients::ConfigError::MissingVariable(var) => ServiceError::MissingVariable(var),
...
impl IntoResponse for ServiceError {
    fn into_response(self) -> Response {
        let (status, error_message) = match &self {
            ServiceError::Io(e) => {
                error!("IO error: {}", e);
                (StatusCode::INTERNAL_SERVER_ERROR, "Internal I/O error".to_string())
            }
            ...
            ServiceError::BsonSerialize(e) => {
                ...
                (StatusCode::INTERNAL_SERVER_ERROR, "Failed to serialize data".to_string())
            }
            ServiceError::BsonDeserialize(e) => {
                error!("BSON deserialization error: {}", e);
                (StatusCode::BAD_REQUEST, "Failed to deserialize data".to_string())
            }
